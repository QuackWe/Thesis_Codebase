...
pickle_train = open(output_dir+"/"+output_dir.split('/')[2]+"_train.pickle","rb")
...
y_train = pd.read_csv(output_dir+"/"+dataset_name+"_train_norm.csv")
...
model_checkpoint = ModelCheckpoint(output_dir+"/model_{epoch:02d}-{val_loss:.2f}.h5", monitor='val_loss', verbose=0,
                                       save_best_only=True, save_weights_only=False, mode='auto')
lr_reducer = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, verbose=0, mode='auto',
                                   min_delta=0.0001, cooldown=0, min_lr=0)

model.fit(X_train, Y_train, epochs=200, batch_size=args.batch_size, verbose=0, callbacks=[early_stopping, lr_reducer], validation_data=(X_val, Y_val))
print("Train complete")
#epochs 200

# Save the final trained model explicitly
model.save(output_dir+"/"+dataset_name+".h5")
print("Final model saved at "+output_dir+"/"+dataset_name+".h5")
